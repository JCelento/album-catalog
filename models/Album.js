...
return db.query("select * from album where album_id=?",[id],callback);
 },
addAlbum:function(Album,callback){
return db.query("insert into album values(?,?,?)",[Album.Id,Album.Name,Album.ArtistId],callback);
 },
deleteAlbum:function(id,callback){
return db.query("delete from album where album_id=?",[id],callback);
 },
updateAlbum:function(id,Task,callback){
